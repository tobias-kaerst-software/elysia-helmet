...
type XPermittedCrossDomainPolicies = (options?: Readonly<XPermittedCrossDomainPoliciesOptions>) => readonly [string, string];

const ALLOWED_PERMITTED_POLICIES = new Set(["none", "master-only", "by-content-type", "all"]);

const getHeaderValueFromOptions = ({ permittedPolicies = "none" }: Readonly<XPermittedCrossDomainPoliciesOptions>): string => {
