...
export const isDirectiveValueInvalid = (directiveValue: string): boolean =>
  /;|,/.test(directiveValue);

export const has = (obj: Readonly<object>, key: string): boolean =>
...
const normalizeDirectives = (
  ...
  for (const rawDirectiveName in rawDirectives) {
    ...
    const directiveValue =
      typeof rawDirectiveValue === 'string'
        ? [rawDirectiveValue]
        : rawDirectiveValue;
...
const getHeaderValue = (directives: Readonly<NormalizedDirectives>) => {
  ...
  directives.forEach((rawDirectiveValue, directiveName) => {
    const directiveValue = rawDirectiveValue.reduce(
      (prev, curr) => `${prev} ${curr}`,
      ''
    );
