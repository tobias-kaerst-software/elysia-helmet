...
export const dashify = (str: string): string => str.replace(/[A-Z]/g, (capitalLetter) => `-${capitalLetter.toLowerCase()}`);

const INVALID_DIRECTIVE_VALUE_REGEX = /;|,/;
export const isDirectiveValueInvalid = (directiveValue: string): boolean => INVALID_DIRECTIVE_VALUE_REGEX.test(directiveValue);

export const has = (obj: Readonly<object>, key: string): boolean => Object.hasOwn(obj, key);

export const getDefaultDirectives: () => Record<string, string[]> = () => ({
  ...DEFAULT_DIRECTIVES,
});

const INVALID_DIRECTIVE_NAME_REGEX = /[^a-zA-Z0-9-]/;
const normalizeDirectives = (options: Readonly<ContentSecurityPolicyOptions>): NormalizedDirectives => {
  ...
  for (const rawDirectiveName in rawDirectives) {
    ...
    if (rawDirectiveName.length === 0 || INVALID_DIRECTIVE_NAME_REGEX.test(rawDirectiveName)) {
